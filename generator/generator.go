...
import (
	...
	wk "github.com/RustLangLatam/protoc-gen-openapi/generator/wellknown"
...
//var statusProtoDesc = (&v3.ErrorMessage{}).ProtoReflect().Descriptor()
...
func (g *OpenAPIv3Generator) buildOperationV3(
	...
	if *g.conf.DefaultResponse {
		...
		statusSchema := wk.NewDefaultRpcStatusSchema(statusSchemaName)
	...
	// If a response body field is specified, we need to pass a message as the response body.
	if responseBodyField != "" {
		var responseSchema *v3.SchemaOrReference

		if responseBodyField == "*" {
			// Pass the entire response message as the response body.
			responseSchema = g.reflect.schemaOrReferenceForMessage(outputMessage.Desc)

		} else {
			// If body refers to a message field, use that type.
			for _, field := range outputMessage.Fields {
				if string(field.Desc.Name()) == responseBodyField {
					switch field.Desc.Kind() {
					case protoreflect.StringKind:
						responseSchema = &v3.SchemaOrReference{
							Oneof: &v3.SchemaOrReference_Schema{
								Schema: &v3.Schema{
									Type: "string",
								},
					...
					case protoreflect.MessageKind:
						responseSchema = g.reflect.schemaOrReferenceForMessage(field.Message.Desc)
			...
			for _, rule := range rules {
				...
				if methodName != "" {
					...
					op, path2 := g.buildOperationV3(
						d, operationID, service.GoName, comment, defaultHost, path, body, inputMessage, responseBody, outputMessage)

					// Merge any `Operation` annotations with the current
					extOperation := proto.GetExtension(method.Desc.Options(), v3.E_Operation)
					if extOperation != nil {
						if resp, ok := extOperation.(*v3.Operation); ok {
							if resp != nil {
...
func (g *OpenAPIv3Generator) addSchemasForMessagesToDocumentV3(d *v3.Document, messages []*protogen.Message) {
	...
	for _, message := range messages {
		...
		oneOf := make([]*v3.SchemaOrReference, 0)
		...
		for _, field := range message.Fields {
			...
			wrapperNeeded := inputOnly || outputOnly || description != "" || field.Oneof != nil

			if wrapperNeeded && field.Oneof != nil && !field.Desc.HasOptionalKeyword() {
				if _, ok := fieldSchema.Oneof.(*v3.SchemaOrReference_Reference); ok {
					oneOf = append(
						oneOf, fieldSchema,
					)
				} else if schema, ok := fieldSchema.Oneof.(*v3.SchemaOrReference_Schema); ok {
					schema.Schema.Description = description
					schema.Schema.Title = field.GoName
					...
					oneOf = append(
						oneOf, &v3.SchemaOrReference{Oneof: schema},
					)
				}
				continue
			}
